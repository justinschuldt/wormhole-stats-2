...
const chainNameColors = Object.entries(chainColors).reduce<{[chainName: string]: string}>(
  // returns an array of hsl colors, indexed by chain name
  (accum, [chain, color]) => {
    accum[chainEnums[Number(chain)]] = color;
    return accum;
  },
  {}
);
